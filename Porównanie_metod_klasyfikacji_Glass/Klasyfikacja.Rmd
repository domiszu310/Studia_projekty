...
```{r glass.na, fig.cap="\\label{fig:glass.na}Wykres przedstawiający procentowy udział wartości brakujących w danych Glass", message=FALSE, warning=FALSE}
...
```{r svm.basic}
svm.basic <- svm.glass(uknn.glass, tknn.glass)
```
...
svm.lin <- svm.glass(uknn.glass, tknn.glass, kernel = "linear")
```
...
svm.poly <- svm.glass(uknn.glass, tknn.glass, kernel = "polynomial")
```
...
svm.sigma <- svm.glass(uknn.glass, tknn.glass, kernel = "sigmoid")
```
...
radial.tune <- tune(svm, Type ~., data = uknn.glass,
...
```{r svm.radial.opt}
svm.radial.opt <- svm.glass(uknn.glass, tknn.glass, C = c.opt.radial, g = gamma.opt.radial)
```
...
linear.tune <- tune(svm, Type ~., data = uknn.glass, # dane uczące bez etykietek klas
...
```{r svm.lin.opt}
svm.lin.opt <- svm.glass(uknn.glass, tknn.glass, kernel = "linear", C = c.opt.lin)
```
...
```{r ulubiona.ostatnia.ostatnia.tabelka, tab.cap="\\label{fig:ulubiona.ostatnia.ostatnia.tabelka}Tabelka pozwalająca porównać wszystkie najmniejsze błędy klasyfikacji i sposoby ich wyznaczenia dla każdego z testowanych algorytów."}
...
Porównując błędy z najlepszej opcji w każdej metodzie największy błąd klasyfikacji otrzymujemy, wykorzystując `r bad` (tabela \ref{fig:bad.ostatnia.ostatnia.tabelka}). Ten błąd wynosi `r bad.error`. Jest to zatem najgorsza metoda dla danych `Glass`.
